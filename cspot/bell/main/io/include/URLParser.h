 ...
 public:
  ...
  static std::string urlDecode(const std::string& value) {
    ...
    for (std::size_t i = 0; i < value.size(); ++i) {
      ...
      // Both characters after '%' must actually be hex digits - without
      // this check, strtol() on a malformed/truncated escape (e.g. "%zz"
      // or a lone trailing "%A") silently produces garbage (0 for no
      // valid digits at all) instead of leaving the input alone. Found
      // porting this from cspot_connect.cpp's own local copy of this
      // function (see docs/dealer_websocket_migration_historial.md §52),
      // which already had this validation - upstreamed here instead of
      // staying duplicated.
