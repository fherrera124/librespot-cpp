...
class BellLogger : public bell::AbstractLogger {
 public:
  ...
  // Each call below is several separate printf()/vprintf()/std::cout
  // calls - without this, two tasks logging at once interleave freely
  // (visible as garbled output) and race on stdio's own internal
  // buffering state. See finding F87.
...
// Caller supplies the submodule tag as the first element of `...` (see
// EncodedAudioStream.cpp for the established call shape: `BELL_LOG(info,
// TAG, "format", args...)`) - this macro itself doesn't hardcode one, so
// each bell/ file can tag its own logs.
