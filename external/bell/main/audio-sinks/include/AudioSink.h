...
class AudioSink {
 public:
  ...
  // Discard any audio already handed to feedPCMFrames() but not yet
  // actually played, so a pause is heard immediately instead of after
  // whatever's buffered downstream drains. Default no-op for sinks with no
  // meaningful buffer to discard.
  virtual void flush() {}
