...
// 2). Public interface is byte-for-byte the same as upstream cspot/bell;
// only the AES-CTR, PBKDF2 and random-bytes implementations changed (see
// Crypto.cpp).
//
// `mbedtls/aes.h` no longer exists as a public header in mbedTLS 4.0, so
// unlike the original this no longer keeps a persistent mbedtls_aes_context
// member - aesCTRXcrypt/aesECBdecrypt each set up their own short-lived
// context per call instead (aesECBdecrypt already did this).
