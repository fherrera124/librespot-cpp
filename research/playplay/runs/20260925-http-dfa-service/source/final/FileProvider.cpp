...
constexpr bool kForcePlayPlay = true;
...
void DefaultFileProvider::taskLoop() {
  if (providedFileSemaphore.take()) {
    ...
    if (!audioKeyResponse) {
      BELL_LOG(warn, LOG_TAG, "Timed out waiting for audio key for track {}",
               file->itemId.uri);
      if (!kForcePlayPlay) {
        file->isError = true;
        eventLoop->post(EventLoop::EventType::FILE_PROVIDED, std::move(*file));
    ...
    if (kForcePlayPlay || !audioKeyResponse->success) {
      ...
      if (licenseRes) {
        auto jsonBody = playplay_service::requestBody(*licenseRes);
        if (jsonBody) {
          bell::HTTPClient httpClient;
          httpClient.operationTimeoutMs = 25000;
          std::string serviceUrl = "http://127.0.0.1:8765/deob";
          if (const char* configuredUrl = std::getenv("PLAYPLAY_SERVICE_URL")) {
            if (*configuredUrl) serviceUrl = configuredUrl;
          }
          bell::http::Headers headers = {{"Content-Type", "application/json"}};
          if (const char* token = std::getenv("PLAYPLAY_SERVICE_TOKEN")) {
            if (*token) headers["X-PlayPlay-Token"] = token;
          }

          auto httpRes = httpClient.post(serviceUrl, headers,
                                         std::string_view(*jsonBody));
          if (httpRes && httpRes->statusCode == 200) {
            auto responseBody = httpRes->text();
            if (responseBody) {
              auto aesKey = playplay_service::parseResponse(*responseBody);
              if (aesKey) {
                file->decryptionKey = std::move(*aesKey);
                ...
                eventLoop->post(EventLoop::EventType::FILE_PROVIDED,
                                std::move(*file));
                return;
              }
            }
          ...
          BELL_LOG(error, LOG_TAG,
                   "PlayPlay deobfuscation failed for track {} (HTTP status {})",
                   file->itemId.uri, httpRes ? httpRes->statusCode : -1);
        } else {
          BELL_LOG(error, LOG_TAG, "Invalid PlayPlay license field sizes");
        }
      } else {
