...
class DefaultSpClient : public SpClient {
 public:
  ...
  bell::Result<PlayPlayLicense> playPlayLicense(
...
bell::Result<PlayPlayLicense> DefaultSpClient::playPlayLicense(
  ...
  if (!nanopb_helper::encodeToVector(request, requestBytes)) {
    ...
    return bell::make_unexpected_errc<PlayPlayLicense>(
        std::errc::bad_message);
  }
  ...
  if (licenseResponse.obfuscatedKey.size() != 16 ||
      licenseResponse.b4Seq.size() != 4) {
    BELL_LOG(error, LOG_TAG, "Invalid PlayPlay license fields");
    return bell::make_unexpected_errc<PlayPlayLicense>(
        std::errc::bad_message);
  }

  return PlayPlayLicense{std::move(licenseResponse.obfuscatedKey),
                         std::move(licenseResponse.b4Seq)};
