...
STOP_FILE = TOOLS / "playplay-service.stop"
...
class FridaDfaBackend:
    ...
    def close(self):
        # Let an active request reach its own deadline before detaching.
        if self.lock.acquire(timeout=self.timeout + 2):
            try:
...
def _make_server(host, port):
    ...
    # Reserve the port before attaching Frida. No requests are served until
    # the verified backend replaces this placeholder and serve_forever starts.
    return ExclusiveServer(host, port, None)


def _main_spotify_pid():
    import psutil
    candidates = []
    for process in psutil.process_iter(["pid", "name", "cmdline"]):
        info = process.info
        args = info["cmdline"]
        if (info["name"] or "").lower() == "spotify.exe" and args and not any(
            arg.startswith("--type=") for arg in args
        ):
            candidates.append(info["pid"])
    if len(candidates) != 1:
        raise ExtractionError("Expected exactly one running main Spotify.exe")
    return candidates[0]


def main(argv=None):
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--pid", type=int, help="main Spotify PID; auto-detected if omitted")
    ...
    if (args.pid is not None and args.pid <= 0) or not 1 <= args.port <= 65535 or not 0 < args.timeout <= 120:
    ...
    try:
        # The local PowerShell stop command creates this marker. A stale marker
        # from a previous stop is removed only after exclusive port ownership.
        STOP_FILE.unlink(missing_ok=True)
        backend = FridaDfaBackend(args.pid or _main_spotify_pid(), timeout=args.timeout)
        server.app = create_app(backend, token)
        import logging
        ...
        watcher.start()
        print(f"PlayPlay API listening on {args.host}:{args.port}", flush=True)
