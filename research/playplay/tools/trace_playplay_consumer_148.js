...
Interceptor.attach(entry,{
    ...
    onLeave(){
        ...
        send({type:'playplay_context',...active});active=null;if(receiverListener!==null)receiverListener.detach();send({type:'done'});
    }
});
...
// 0x640844 dispatches the wrapped key to a virtual receiver at [self+0xc0].
let receiverListener=null;
Interceptor.attach(mod.base.add(0x640844),{
    onEnter(args){
        if(active===null || active.thread!==this.threadId)return;
        const receiver=args[0].add(0xc0).readPointer();
        if(receiver.isNull())return;
        const method=receiver.readPointer().add(0x10).readPointer();
        active.consumer=rva(method);
        active.consumer_prefix=hex(method.readByteArray(48));
        active.consumer_file_id=hex(args[2].readByteArray(20));
        receiverListener=Interceptor.attach(method,{
            onEnter(inner){
                if(active===null || active.thread!==this.threadId)return;
                active.dispatched_key=hex(inner[3].readByteArray(28));
                active.dispatched_file_id=hex(inner[2].readByteArray(20));
                active.key_kind=inner[4].readU32();
                active.key_flags=inner[5].readU32();
            }
        });
        Interceptor.flush();
    }
});
send({type:'attached',handler_rva:'0x4a0494',mode:'trace_playplay_consumer'});
