...
setImmediate(() => {
    try {
        ...
        const streamTarget = new NativeFunction(streamAddress, 'void', ['pointer', 'pointer'], options);
            ...
            transform: function (iterator) {
                ...
                while (instruction !== null) {
                    if (instruction.address.compare(streamAddress) >= 0 && instruction.address.compare(streamAddress.add(0x1000)) < 0) {
                        ...
                        if ((mnem === 'mov' || mnem === 'xor') && opstr.indexOf('[') !== -1) {
                            const addr = instruction.address.sub(mod.base).toString(16);
                            iterator.putCallout((ctx) => {
                                events.push(addr + ": " + mnem + " " + opstr);
                            });
                        }
                    }
                    iterator.keep();
        ...
        streamTarget(context, output);
        Stalker.unfollow(Process.getCurrentThreadId());
        
        send({type: 'stalker', events: events.slice(0, 100)}); 
        send({type: 'done'});
